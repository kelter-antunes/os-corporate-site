...
reposition:function(e,object){var obj=object||F.current,wrap=obj&&obj.wrap,pos;if(F.isOpen&&wrap){pos=F._getPosition(obj);if(e===false||e&&e.type==="scroll")wrap.stop(true).animate(pos,200).css("overflow","visible");else wrap.css(pos)}},update:function(e){var type=e&&e.type,timeNow=getTime(),current=F.current,width;if(!current||!F.isOpen)return;if(type==="scroll"){if(F.wrap.outerHeight(true)>F.getViewport().h)return;if(F.didUpdate)clearTimeout(F.didUpdate);F.didUpdate=setTimeout(function(){F.reposition(e);
F.didUpdate=null},50);return}if(F.lock)F.lock.css("overflow","hidden");F._setDimension();F.reposition(e);if(F.lock)F.lock.css("overflow","auto");if(current.caption.type==="float"){width=F.getViewport().w-(F.wrap.outerWidth(true)-F.inner.width());current.caption.wrap.css("width",width).css("marginLeft",(width*.5-F.inner.width()*.5)*-1)}if(current.expander)if(current.canShrink)$(".fancybox-expand").show().attr("title",current.locales[current.locale].SHRINK);else if(current.canExpand)$(".fancybox-expand").show().attr("title",
